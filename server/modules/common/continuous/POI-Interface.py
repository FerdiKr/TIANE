...
def poi_image(frame, x, y, w, h, sub_type):
    ...
    #if not (y-20) <= 0:
    y -= 20
    #if not (x+w+60) >= frame.shape[1]:
    w += 60
    #else:
        #w = frame.shape[1] - x
    #if not (y+h+60) >= frame.shape[0]:
    h += 60
    #else:
        #h = frame.shape[0] - y
...
def run(tiane, profile):
    ...
    for roomname, camlist in profile['TIANE_face_boxes_names'].copy().items():
        if not camlist == {}:
            profile['TIANE_prev_room_camframes'][roomname] = camlist
    ...
    for roomname in profile['TIANE_prev_roomnames']:
        if not roomname in roomnames:
            if profile['TIANE_room_cam_timeout'][roomname] <= 25:
                for cam in profile['TIANE_prev_room_camframes'][roomname].values():
                    cam.append(roomname)
                    cams.append(cam)
    ...
    if not cams == []:
        ...
        if len(cams) == 1:
            ...
            for top,right,bottom,left,name in cams[0][2]:
                x = left + 300
                y = top + 200
                w = right - left
                h = bottom - top
            ...
            for cam in cams:
                ...
                if i == 0:
                    ...
                    for frame in frames[:i]:
                        x += frame.shape[1]
                    for top,right,bottom,left,name in cams[i][2]:
                        x = left + 300 + x
                        y = top + 200
                        w = right - left
                        h = bottom - top
