...
WORDS = ['Hogwarts', 'Gryffindor', 'Ravenclaw', 'Hufflepuff', 'Slytherin', 'Dumbledore']
PRIORITY = 1

def reply(text, tiane):
    output = ''
    text = text.lower()
    t = str.split(text)
    ind = 1
    ...
    if 'gryffindor' in text:
        ...
        if 'in welchem hogwarts haus bist du' in text:
            ...
            output = random.choice(['Wie würdest du dich denn einschätzen?', 'In welchem Haus wärst du denn gerne?', 'Wo wärst du denn gerne?', 'Wo ist denn dein Lieblingscharakter?'])
            a = True
