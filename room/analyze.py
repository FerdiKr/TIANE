...
class Sentence_Analyzer:
            ...
            for i, w in Monate.items():
                if w in text:
                    ...
                    for ind, word in satz.items():
                        if word in Monate.values():
                            m = word
                            for ind, word in Monate.items():
                                if word == m:
                                    month = ind
                                    if month <= 9:
                                        month = '0' + str(month)
    ...
    def get_wochentag_rel(self, text):
        now = datetime.datetime.now()
...
def main():
    ...
    eingabe = 'Mach das Licht in der Küche an'
